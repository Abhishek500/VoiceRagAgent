...
async def bot_connect(request: Request) -> Dict[str, Any]:
    ...
    prompt_type: str = body.get("prompt_type", "")
    ...
    if prompt_type:
        qs["prompt_type"] = prompt_type

    query_str = urllib.parse.urlencode(qs) if qs else ""
    ws_url = f"{ws_scheme}://{host}/api/v1/stream/ws/{equipment_id}"
    if query_str:
        ws_url = f"{ws_url}?{query_str}"
...
async def websocket_endpoint(websocket: WebSocket, equipment_id: str):
    ...
    try:
        ...
        prompt_type = websocket.query_params.get("prompt_type")
